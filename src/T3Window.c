...
T3Window * t3window_create(const char ** set1, uint8_t count1,
	...
	text_layer_set_font(w->inputLayer, fonts_get_system_font(FONT_KEY_GOTHIC_24));
...
void _t3_updateInverter(T3Window * window) {
    ...
    uint8_t index = (window->row - 1) * 3 + c;
    TextLayer * layer = window->buttons[index];
    text_layer_set_background_color(layer, GColorBlack);
    index = (window->row - 1) * 3 + (window->col - 1);
    layer = window->buttons[index];
